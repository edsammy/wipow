...
void normalread(){  
 while (!MaximWrite("00E", "60")){}          // These lines gets the DSP ready to read Neutral Current.
 ...
 while (!MaximWrite("004", "0000")){}	// Clear Interrupt flag register
  ...
  char mysqlheader[] = "insert into cogen (temp,freq,volta,voltb,voltc,curra,currb,currc,currn,reala,realb,realc,reaca,reacb,reacc,appaa,appab,appac,pfa,pfb,pfc,pft,phaseab,phaseac,thda,cha1,cha2,cha3,cha4,cha5,vha1,vha2,vha3,vha4,vha5,thdb,chb1,chb2,chb3,chb4,chb5,vhb1,vhb2,vhb3,vhb4,vhb5,thdc,chc1,chc2,chc3,chc4,chc5,vhc1,vhc2,vhc3,vhc4,vhc5) values(";
  //char mysqlheader[] = "insert into cogen (temp,freq,volta,voltb,voltc,curra,currb,currc,currn,reala,realb,realc,reaca,reacb,reacc,appaa,appab,appac,pfa,pfb,pfc,pft,phaseab,phaseac) values(";
  SpiSerial.write(mysqlheader, String(mysqlheader).length());
  
  int analogtemp = analogRead(A4);
  double temp = 164.8161486877*exp(-0.3272031214*0.0048828125*analogtemp);
  dtostrf(temp,10,3,buf);
  SpiSerial.write(buf);
  SpiSerial.write(",",1);
