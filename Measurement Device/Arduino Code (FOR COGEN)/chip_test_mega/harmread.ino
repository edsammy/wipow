...
void harmread(){ 
 ...
 //char mysqlheader[] = "insert into cogen (thda,cha1,cha2,cha3,cha4,cha5,vha1,vha2,vha3,vha4,vha5,thdb,chb1,chb2,chb3,chb4,chb5,vhb1,vhb2,vhb3,vhb4,vhb5,thdc,chc1,chc2,chc2,chc3,chc4,chc5,vhc1,vhc2,vhc3,vhc4,vhc5) values(";
...
for(order=1;order<6;order++){    //Current A Harm
  ...
  current_a_harm[order] = (float(((unsigned long)lower_read >> 16) | (upper_read << 16)))*(65536)/100000000;  // LSBs are 10^-8
...
for(order=1;order<6;order++){  // Voltage A Harm
  ...
  dspready();
  MaximRead("830",8);
  voltage_a_harm[order] = (float(((unsigned long)lower_read >> 16) | (upper_read << 16)))*(65536)/10000000;  // LSBs are 10^-7
...
for(order=1;order<6;order++){    // Current B Harm
  ...
  dspready();
  MaximRead("840",8);
  current_b_harm[order] = (float(((unsigned long)lower_read >> 16) | (upper_read << 16)))*(65536)/100000000; 
...
for(order=1;order<6;order++){  // Voltage B Harm
  ...
  dspready();
  MaximRead("830",8);
  voltage_b_harm[order] = (float(((unsigned long)lower_read >> 16) | (upper_read << 16)))*(65536)/10000000; 
...
for(order=1;order<6;order++){    // Current C Harm
  ...
  dspready();
  MaximRead("840",8);
  current_c_harm[order] = (float(((unsigned long)lower_read >> 16) | (upper_read << 16)))*(65536)/100000000; 
...
for(order=1;order<6;order++){  // Voltage C Harm
  ...
  dspready();
  MaximRead("830",8);
  voltage_c_harm[order] = (float(((unsigned long)lower_read >> 16) | (upper_read << 16)))*(65536)/10000000; 
