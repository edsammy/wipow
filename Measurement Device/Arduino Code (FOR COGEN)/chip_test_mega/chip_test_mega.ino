...
void setup() {
  ...
  while(!MaximRead("030",2)){};
  Serial.print("Averaging Filter: ");
...
void loop() {
  ...
  while(SpiSerial.available() > 0) {	// Flushes out the HTTP headers. (Or whatever else header specific to the connection)
    ...
    normalread();	// This calls "normalread.ino" Read that file for more info. Basically, this starts transmitting the "regular (no harmonics)" data.
    delay(500);  
    //SpiSerial.write(41);  // ")"
    //SpiSerial.write(59);  // ";"  To finish the MySQL statement that normalread was pushing out (IF "harmread()" IS DISABLED/COMMENTED OUT)
	
    harmread();		// Like normalread(), this starts transmitting the harmonic data. It also finishes the MySQL statement.
